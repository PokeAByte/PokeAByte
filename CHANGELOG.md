...
- Made a small optimization in game memory management, reducing CPU work in the gen 3, 4 and 5 mappers.
- Updated the JavaScript engine for a moderate performance boost.
...
- The general application settings are no longer read from `appsettings.json`  but instead stored in the Poke-A-Byte configuration directory (where the mappers are also stored). 
