...
public interface IMemoryNamespace
{
    ...
    /// <summary>
    /// Read bytes for the specified memory area as a raw byte array instead of an <see cref="IByteArray"/>.
    /// </summary>
    /// <param name="memoryAddress"> Starting address of the target memory. </param>
    /// <param name="length"> Lenght of the target memory. </param>
    /// <returns> The memory bytes. </returns>
    ...
    /// <summary>
    /// Returns all bytes stored in the memory namespace.
    /// </summary>
    /// <returns> The raw memory byte array. </returns>
    byte[] GetAllBytes();
}
