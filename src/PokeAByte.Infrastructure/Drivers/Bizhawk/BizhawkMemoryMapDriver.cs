...
namespace PokeAByte.Infrastructure.Drivers.Bizhawk
{
    public class BizhawkMemoryMapDriver : IPokeAByteDriver, IBizhawkMemoryMapDriver
    {
        ...
        private Dictionary<string, MemoryMappedViewAccessor> _memoryAccessors = [];
        ...
        private byte[] ReadBizhawkData()
        {
            try
            {
                ...
                _memoryAccessor.ReadArray(0, _readBuffer, 0, DATA_Length);
                return _readBuffer;
            }
            catch (FileNotFoundException ex)
            {
